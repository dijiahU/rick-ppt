...
export class Assets {
 ...
 url(key:string){const a=this.scene.assets?.[key];return allowedUrl(a?.path??key,this.base,this.scene.runtimeOptions?.networkAllowlist??[]);}
 async verify(){for(const [key,asset]of Object.entries(this.scene.assets??{})){const r=await fetch(this.url(key),{redirect:'error',credentials:'omit'});if(!r.ok)throw new Error(`Asset unavailable: ${key}`);const declared=Number(r.headers.get('content-length')??0);if(declared>536870912)throw new Error('Asset too large');const b=await r.arrayBuffer();if(b.byteLength>536870912||asset.bytes!==undefined&&b.byteLength!==asset.bytes)throw new Error(`Asset size mismatch: ${key}`);if(asset.sha256&&await digest(b)!==asset.sha256)throw new Error(`Asset hash mismatch: ${key}`);}}
