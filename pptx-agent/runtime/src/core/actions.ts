...
export class ActionDispatcher {
 ...
 async run(actions:Action[],event:Dict={},locals:Dict={},shared=false):Promise<void>{if(this.disposed)return;if(!this.depth&&!shared)this.count=0;this.depth++;try{for(const action of actions){if(++this.count>this.maximum)throw new Error('Action transaction limit exceeded');try{await this.execute(action,event,locals);}catch(error){throw withDiagnostic(error,{phase:'action',nodeId:action.target??event.target,...(action.type==='plugin'?{plugin:action.name?.split('.')[0]}:{})});}}}finally{this.depth--;}}
 ...
 case 'call':{const result=this.store.expressions.call(a.name!,this.store.expressions.value(a.args??[],env),env);if(a.result)this.store.set(a.result,result);break;}
