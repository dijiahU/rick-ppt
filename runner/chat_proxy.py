...
import contextlib,json,secrets,subprocess,threading,time,urllib.parse
...
class ChatProxy:
 ...
 def complete(self,body):
  ...
  if process.returncode:raise UpstreamError(502,'Upstream transport failed (curl '+str(process.returncode)+')')
  raw,status=process.stdout.rsplit('\n',1);status=int(status)
  if status!=200:
   self.calls.append({'ok':False,'upstream_status':status})
   raise UpstreamError(status if 400<=status<600 else 502,'Provider rejected request (HTTP '+str(status)+')')
  reply=json.loads(raw);response=chat_to_response(reply,mapping,model=self.profile['model'])
  self.calls.append({'ok':True,'seconds':round(time.monotonic()-started,3),'usage':reply.get('usage'),
