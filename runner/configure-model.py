...
p=argparse.ArgumentParser();p.add_argument('--base-url',required=True);p.add_argument('--model',required=True);p.add_argument('--out',type=Path,default=Path(__file__).with_name('model-profile.local.json'));a=p.parse_args()
...
data={'model':a.model,'base_url':a.base_url,'wire_api':'responses','api_key':key,'web_search':'disabled','image_generation':False}
