...
| Y04 | `yolo-dfl`: reduced four-bin illustration probabilities `[.1,.2,.3,.4]`; separately inspect16-bin model view. | Expectation2; explicit four-bin simplification label. Full model support is bins0–15. Probabilities sum1. |
...
| Y07 | `yolo-nms`: confidence.5, class-aware; threshold.3→.5. | Kept IDs `[A,C]→[A,B,C]` in score order. Show B's suppression reason only in first setting. |
