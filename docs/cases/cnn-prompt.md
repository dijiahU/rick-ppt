# CNN acceptance case: authoring brief

Status: researched brief, ready for the post-smoke website run. This file does not
record a submitted job, generated presentation or passed acceptance case. Submit
only after the parent workflow's smoke gate passes; use Rick's existing account.

## Website fields

- Title: `介绍cnn — Interactive CNN from First Principles`
- Language: `en` (the saved original task language).
- Slides: **20**, matching the website outline and final native slide count.
- Audience: understands ordinary neural-network forward propagation and
  backpropagation, but has not learned convolutional networks.
- Mode: native editable PowerPoint with interactive teaching regions.

The original request asked for 50 pages. Rick subsequently allowed fewer pages
when interaction explains the material clearly. Twenty pages retain the complete
reasoning while a learner can inspect multiple patches, channel combinations,
gradient paths and code runs on the same page. Do not compress by dropping the
derivation or replacing the complete implementation with library calls.

## Prompt to submit

Create an English teaching presentation titled **CNNs from First Principles:
...
and reset. Use the shared [acceptance checklist](acceptance-checks.md). Deliver the
...
PowerPoint playback passed based only on browser screenshots.
